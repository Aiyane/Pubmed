...
def getRes(word):
    ...
    for children in AST.children:
        if getLine(children, pattern):
            try:
                with open("C:\\Users\\Administrator\\Desktop\\deal\\res2\\" + word + ".txt", "a", encoding="utf8") as f:
                    for token in children.children:
                        if isinstance(token, my_token.TimeToken):
                            ...
                            if gene_name:
                                token.content = re.sub(r'(Glyma\d{2}[Gg]\d+(\.\d*)?)', "基因>" + gene_name.group(1) + "<基因", token.content, re.IGNORECASE)
                ...
                print(word + ".txt读入: " + token.__name__ + " 出错")
